...
error.prototype.on = function(key, withName) {
	var self = this;
	key = key || '';
	var store = self._messages[key]; 
	if( store && store.length ) {
		var messages = []
		, name = withName ? ( self._alias[key] || key ) : '';
		name = name ? (name + " ") : "";
		for (var i = 0, l = store.length; i < l; i++) {
			var msg = store[i];
			if( msg instanceof error ) {
				merge(messages, msg.messages(withName));
			} else {
...
error.prototype.messages = function(withName) {
	var messages = [];
	for (var key in this._messages) {
		merge(messages, this.on(key, withName));
	};
